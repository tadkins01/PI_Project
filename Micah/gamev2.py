...
class SnakeGame:
    ...
    def prompt_choice() -> str:
        while True:
            cprint("\n  Direction to slither:", C.CYAN)
            cprint("    [W] Up", C.WHITE)
            cprint("    [A] Left", C.WHITE)
            cprint("    [S] Down", C.WHITE)
            cprint("    [D] Right", C.WHITE)
            raw = input(f"\n  {C.BOLD}> {C.RESET}").strip().lower()
            if raw in SnakeGame.MOVES:
                return SnakeGame.MOVES[raw]
            cprint("  ⚠  Please enter W, A, S, or D.", C.YELLOW)

    @staticmethod
    def resolve(choices: dict) -> dict:
        """Players win points if they move to a unique direction without colliding."""
        counts = {}
        for move in choices.values():
            counts[move] = counts.get(move, 0) + 1
        
        winners = [p for p, m in choices.items() if counts[m] == 1]
        return {"choices": choices, "winners": winners}

    @staticmethod
    def format_result(result: dict, my_name: str) -> str:
        choices = result["choices"]
        winners = result["winners"]
        lines = [f"\n  {C.BOLD}── Snake Pit Results ──{C.RESET}\n"]
        
        for player, move in choices.items():
            tag = " ← you" if player == my_name else ""
            if player in winners:
                ...
                mark = f"{C.RED}💥"
                note = "(Collision!)"
            lines.append(f"  {mark}{C.RESET}  {player} moved {C.BOLD}{move}{C.RESET} {C.DIM}{note}{tag}{C.RESET}")
        
        if len(winners) == 0:
            lines.append(f"\n  {C.RED}Everyone collided! No points.{C.RESET}")
        elif my_name in winners:
            lines.append(f"\n  {C.GREEN}{C.BOLD}🍎 You found space and grew!{C.RESET}")
        else:
            lines.append(f"\n  {C.RED}You crashed into another snake!{C.RESET}")
